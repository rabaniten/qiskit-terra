...
class UCY(UCRot):  # pylint: disable=abstract-method
    ...
    q_controls =  list of control k qubits (at least of length one). The qubits are ordered according to their
                    ...
                    For example if q_controls=[q[2],q[1]] (with q = QuantumRegister(2)), the rotation R_(a_0)is
                    performed if q[2] and q[1] are in the state zero,
                    R_y(a_1) is performed if q[2] is in the state one and q[1] is in the state zero, and so on.
