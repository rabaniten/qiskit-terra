...
around the Z-axes (i.e., uniformly controlled R_y rotations).
...
class UCY(UCRot):  # pylint: disable=abstract-method
    """Uniformly controlled R_y rotations (also called multiplexed rotations). The decomposition is based on
    ...
    q_controls = list of control k qubits (at least of length one). The qubits are ordered according to their significance in the computational basis.
                    For example if q_controls=[q[2],q[1]] (with q = QuantumRegister(2)), the rotation R_y(a_0)is performed if q[2] and q[1] are in the state zero,
                    R_y(a_1) is performed if q[2] is in the state zero and q[1] is in the state one, and so on.
    ...
    def __init__(self, angle_list, q_controls, q_target, circ = None):
        ...
        self._dec_uc_rot_gate()ßß


def ucy(self, angle_list, q_controls, q_target):
    return self._attach(UCY(angle_list, q_controls, q_target, self))
